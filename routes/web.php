...
Route::post('/login', array('uses' => 'Site\SiteUserController@doLogin', 'as' => 'user.dologin'));
Route::post('/register', array('uses' => 'Site\SiteUserController@doRegister', 'as' => 'user.doregister'));
...
Route::group(['prefix' => 'member'], function () {
	Route::get('/{uid}/bang-diem', ['as' => 'memeber.history', 'uses' => 'Site\SiteMemberController@history'])
		->where('uid', '[0-9]+');
		
	Route::get('/{uid}/tien-trinh', ['as' => 'memeber.history.score', 'uses' => 'Site\SiteMemberController@historyScore'])
		->where('uid', '[0-9]+');

	Route::get('/{uid}/lich-su-lam-bai', ['as' => 'memeber.history.question', 'uses' => 'Site\SiteMemberController@historyQuestion'])
		->where('uid', '[0-9]+');

	Route::get('/{uid}/profile', 'Site\SiteMemberController@profile')
		->where('uid', '[0-9]+');
	Route::post('/{uid}/profile', 'Site\SiteMemberController@saveProfile')
		->where('uid', '[0-9]+');
});
...
Route::get('{grade_slug}/{subject_slug}', array(
	'uses' => 'Site\SiteSubjectController@show',
	'as' =>'showsubject'
));

Route::get('{grade_slug}/{subject_slug}/{lession_slug}', array(
	'uses' => 'Site\SiteLessionController@show',
	'as' =>'showlession'
));
