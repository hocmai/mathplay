		...
		<div class="input-group inline-block">
			...
			<select name="type" class="form-control selectpicker" data-live-search="true">
				...
				@foreach( CommonQuestion::getAllType() as $index => $value )
					<option{{ ($index == request()->get('type')) ? ' selected' : '' }} data-tokens="{{ str_slug($value, ' ').' '.$value }}" value="{{ $index }}">{{ $value }}</option>
				@endforeach
			</select>
		</div>
		<div class="input-group inline-block">
			<label style="display: block;">Chuyên đề</label>
		  	{{ Form::select('chapter', ['' => '--Tất cả--'] + Common::getChapterList(), request()->get('chapter'), ['class' => 'form-control', 'style' => 'max-width: 200px']) }}
		</div>
		<div class="input-group inline-block">
			<label style="display: block;">Môn học</label>
		  	{{ Form::select('subject', ['' => '--Tất cả--'] + Common::getSubjectList(), request()->get('subject'), ['class' => 'form-control']) }}
		</div>
		<div class="input-group inline-block">
			<label style="display: block;">Lớp</label>
		  	{{ Form::select('grade', ['' => '--Tất cả--'] + Common::getGradeList(), request()->get('grade'), ['class' => 'form-control']) }}
		</div>
		<div class="input-group inline-block">
