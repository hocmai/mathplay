...
<div class="form-group">
	...
	{{ Form::number('question_config[num_value][]', !empty($config['num_value']) ? $config['num_value'] : 5, ['class' => 'form-control']) }}
	<em>Có bao nhiêu số trong dãy số? Nhập "0" để lấy ngẫu nhiên từ 5 - 15</em>
</div>
<div class="form-group">
	{{ Form::label('', 'Số nhỏ nhất') }}
	{{ Form::number('question_config[min_value][]', !empty($config['min_value']) ? $config['min_value'] : 1, ['class' => 'form-control']) }}
	<em>Nhập "0" để lấy ngẫu nhiên 1 số > 1</em>
</div>
<div class="form-group">
	{{ Form::label('', 'Số lớn nhất') }}
	{{ Form::number('question_config[max_value][]', !empty($config['max_value']) ? $config['max_value'] : 5, ['class' => 'form-control']) }}
